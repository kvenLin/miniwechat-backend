...
public class DefaultRandomCodeStrategy implements RandomCodeStrategy {
    ...
    private boolean createBloomFilter() {
        ...
        if (codesFilter == null) {
            log.info("create new bloom filters");
            codesFilter = new BitSet(MAX_BITS); // 2^24
        } else {
            int size = codesFilter.cardinality();
            if (size >= MAX_BITS) {
                log.warn("bloom filters with prefix file {} is already full", codePrefixIndex);
                return false;
            }
            log.info("recreate bloom filters with cardinality {}", size);
        }
